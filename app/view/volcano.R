...
box::use(
  shiny[moduleServer, NS, plotOutput, renderPlot, plotlyOutput, renderPlotly, 
        reactive, observeEvent, req, div, span, p, tags],
  plotly[plot_ly, layout, event_data, add_markers, add_annotations, config, 
         toRGB, highlight_key],
...
ui <- function(id, height = "600px", width = "100%") {
  ...
  div(
    ...
    div(
      ...
      p(
        span(tags$b("Hover"), "over points to see details. ", 
  ...
  moduleServer(id, function(input, output, session) {
    ...
    plot_data <- reactive({
      ...
      if (!all(required_cols %in% names(data))) {
        ...
        stop("Missing required columns in dataset: ", paste(missing, collapse = ", "))
      }
    ...
    output$volcano_plot <- renderPlotly({
      ...
      p <- plot_ly(data = data, source = "volcano") %>%
        add_markers(
          ...
          text = ~paste(
            "Gene:", genes, "<br>",
            "log2(FC):", round(log2FC, 3), "<br>",
            "p-value:", signif(pval, 3), 
            if("qvalue" %in% names(data)) paste("<br>q-value:", signif(data$qvalue, 3)) else ""
          ),
          hoverinfo = "text"
        ) %>%
        layout(
          ...
          shapes = list(
            ...
            list(
              type = "line", 
              x0 = log2(fc_cutoff()), x1 = log2(fc_cutoff()), 
              y0 = 0, y1 = max(data$neg_log_pval, na.rm = TRUE),
            ...
            list(
              type = "line", 
              x0 = min(data$log2FC, na.rm = TRUE), x1 = max(data$log2FC, na.rm = TRUE),
              y0 = -log10(pval_cutoff()), y1 = -log10(pval_cutoff()),
              line = list(color = "grey", dash = "dash")
            )
          )
        ) %>%
        config(
          ...
          modeBarButtonsToRemove = c(
            "sendDataToCloud", "editInChartStudio", "lasso2d", "autoScale2d"
          )
        )
      ...
      if (!is.null(highlight_info())) {
        ...
        if (!is.null(info) && !is.null(highlight_genes()) && length(highlight_genes()) > 0) {
          ...
          annotation_text <- paste0(
            ...
            "<br>", length(highlight_genes()), " genes"
          )
